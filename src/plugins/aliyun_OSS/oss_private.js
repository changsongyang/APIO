...
// web访问地址：https://private.zfm18.com/ + object_key
...
export let listFiles = client.list()
 ...
 * 用法示例：
 ...
 return update_local_file("test.js", path.join(__dirname, "address.js")).then(function (result) {
        console.log(result);
    }).catch(function (err) {
        console.log(err);
    });
 ...
 */
export let updateLocalFile = (objectKey, localFile) => {
 ...
 * @param objectKey
 * @param localFile
 * @param chunked 是否使用chunked encoding
 * @returns {Object}
 */
export let updateStream = (objectKey, localFile, chunked) => {
  // // use 'chunked encoding'
  let stream = fs.createReadStream(localFile)
  if (chunked) {
    return client.putStream(objectKey, stream)
  } else {
    let size = fs.statSync(localFile).size // don't use 'chunked encoding'
    return client.putStream(objectKey, stream, {
      contentLength: size
    })
  }
 ...
 */
export let updateBuffer = (objectKey, buffer) => {
 ...
 * @param localFile
 */
export let updateMultipart = (objectKey, localFile) => {
 ...
 * @param localFile
 */
export let updateMultiparts = (objectKey, localFile) => {
  let checkpoint
  return client.multipartUpload(objectKey, localFile, {
 ...
 */
export let downloadLocalFile = (objectKey, localFile) => {
  return client.get(objectKey, localFile)
}

// export let download_stream = (object_key, local_file) => {
...
/***
 * 获取私有文件下载地址/图片访问地址
 * @param objectKey 文件名称，在数据库里面叫name
 * @param oldtime 过期时间，建议设置1800s，就是半小时。
 */
export let getFileUrl = (objectKey, oldtime) => {
  if (objectKey === '') {
    return false
  } else {
    return client.signatureUrl(objectKey, {
      expires: oldtime
    }).replace('http://zfm18private.oss-cn-shenzhen.aliyuncs.com/', 'https://private.zfm18.com/')
  }
}
